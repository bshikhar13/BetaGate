	...
	public static void main(String[] args) {
		...
		try {
			...
			DataStore sds = Factory.createDataStore(
					"gate.persist.SerialDataStore", SERIAL_DATASTORE_PATH);
			sds.open();
			...
			// 5.1. create Index definition that tells the IR engine how to
			// index the corpus
			DefaultIndexDefinition did = new DefaultIndexDefinition();
			// 5.1. use the Lucene IR engine (the only option at present)
			// may be changed in the future
			did.setIrEngineClassName(gate.creole.ir.lucene.LuceneIREngine.class
					.getName());
			// 5.2. specify index location - this is different from the location
			// of the serialized corpus
			did.setIndexLocation(INDEX_LOCATION);
			// 5.3. specify fields to be indexed and their respective Readers
			// if the field being indexed is document feature then set NULL as
			// the default
			// FieldReader would search the document features for a feature with
			// the same name and will
			// index its value (expected String)
			did.addIndexField(new IndexField("author", null, false));
			// for the document content specify a predefined FieldReader called
			// DocumentCOntentReader
			// that will index the content of the document
			did.addIndexField(new IndexField("content",
					new DocumentContentReader(), false));
			// any other things to be indexed such as custom annotations and
			// their features
			// ...require a custom FieldReader to be created and spceified in
			// the same manner

			// 5.4 finally tell the indexed corpus (that will be created) to use
			// the above
			// index definition
			indexedCorpus.setIndexDefinition(did);

			// 5.5 ask the IndexManager to create the index (delete it
			// beforehand if already existing)
			indexedCorpus.getIndexManager().deleteIndex();
			indexedCorpus.getIndexManager().createIndex();

			// now we have the two documents indexed and FTS queries may be
			// specified for them
			// using the respective IR manager (Lucene) search syntax

			// 6. (optionally) optimize index
			// with time indexes become suboptimal with additions/removal of new
			// documents
			// optimize the index from time to time for better performance
			// on large indexes this will take some time since it involves index
			// recreation
			...
			// ...this query looks for documents that has "author" field equal
			// to "foundation"
			// and contain the phrase "until there's a cure" in the content (the
			// until.org page used
			...
			while (it.hasNext()) {
				...
				Document resultDoc = (Document) sds.getLr(
						"gate.corpora.DocumentImpl", qr.getDocumentID());
				Out.prln("Query1: DOC_NAME=" + resultDoc.getName());
				Out.prln("Query1: score = " + score);
				Out.prln("Query1: author = "
						+ resultDoc.getFeatures().get("author"));
				Out.prln("------------");
